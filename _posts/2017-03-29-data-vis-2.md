...
We can see pretty clear trends over time, and so we try to plot a simple straight line through this using `stat_smooth` in ggplot, by specifying a linear model (lm) method. 
